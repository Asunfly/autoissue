# AionUi Issue Agent（最简分支：固定仓库 iOfficeAI/AionUi，v23）
...
- 每次提交或附件准备都只读取“当前这个 `work_order.json`”里的 `attachments`，不会扫描其他 `work_id` 的历史附件。
- 推荐目录结构：
...
- 附件：优先保留用户提供的截图/录屏/日志路径；若本地附件需要跟随 issue 一起发布，优先先生成/回写 `attachment_markdown`（GitHub 已托管的图片链接），再提交。
...
## 提交方式（Skill / github_mcp / chrome_mcp）选择与兜底
- 默认：`submit_method = "skill"`
- 用户明确要求 GitHub MCP：`submit_method = "github_mcp"`
- 用户明确要求浏览器 MCP：`submit_method = "chrome_mcp"`
- Skill 失败且用户仍要求发布：
  - 需要“可见浏览器 + 表单直填/补传附件”时切 `chrome_mcp`
  - 需要“直接用 GitHub MCP 创建 issue”时切 `github_mcp`
...
- **github_mcp（GitHub API/MCP）**：不依赖浏览器表单，直接用 GitHub MCP 创建 issue。若存在本地截图，需先产出 `attachment_markdown` 后再创建 issue。

## work_order.json 生成规范（Skill / github_mcp / chrome_mcp 共用）
- 无论走哪条提交链路，都先生成 `work_order.json`，作为统一的结构化数据源。
...
- 固定字段（总是生成）：
  ...
  - `issue_url`: `""`
- 可选附件衍生字段：
  - `attachment_markdown`: 已上传到 GitHub 后生成的 Markdown 片段（例如 `![image](https://github.com/user-attachments/assets/...)`）
...
## 附件策略（本地截图/录屏/日志）
- `attachments` 始终保留原始本地路径，作为事实来源。
- `attachments` 可以是绝对路径，也可以是相对 `work_order.json` 的路径；当前实现不会假设附件一定放在 `work_id` 目录内。
- 如果要让截图真正出现在 GitHub issue 中，不要指望 GitHub MCP 直接吃本地文件路径；先通过浏览器会话把文件上传成 GitHub 可访问 URL，并写回 `attachment_markdown`。
- 预上传只会处理 GitHub 当前支持的图片附件：`.png`、`.gif`、`.jpg`、`.jpeg`，且单文件不超过 `10MB`。
- 不支持或超限的附件会在上传前被过滤，不会阻断整体提交；脚本会保留本地路径，并把跳过原因写入 `Additional Context` 和 `events[]`。
- 现有脚本支持 `--prepare-attachments-only`：只打开 GitHub Issue Form，向 `Additional Context` 上传附件并把生成的 `attachment_markdown` 回写到 `work_order.json`，不创建 issue。
- `github_mcp` 创建 issue 时优先读取 `attachment_markdown`；没有时只能退化为“在正文中列出本地路径”，并明确提示用户附件尚未真正上传。

## github_mcp 提交流程（当 submit_method="github_mcp"）
目标：不用浏览器表单，直接通过 GitHub MCP 创建 issue。

### 操作步骤
1) 先生成 `work_order.json`
2) 若 `attachments` 包含本地截图/录屏：
   - 优先执行附件预处理，产出 `attachment_markdown`
   - 推荐命令：`bash run_macos_linux.sh /path/to/work_order.json --prepare-attachments-only`
3) 使用 `scripts/python/build_github_mcp_payload.py --work-order /path/to/work_order.json` 生成 GitHub MCP 需要的 `title/body`
4) 调用 GitHub MCP 创建 issue
5) 创建成功后，将返回的 `issue_url` / `issue_number` 写回 `work_order.json`
...
- 用户仍坚持发布：根据用户要求切换到 `github_mcp` 或 `chrome_mcp` 完成提交。
